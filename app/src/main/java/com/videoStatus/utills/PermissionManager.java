...
/**
 * Created by shootme-hardik on 5/5/17.
 */
...
public class PermissionManager {
    ...
    @TargetApi(23)
    public boolean checkAndRequestPermissions(String permission, int serviceCode) {
    ...
    public boolean checkAndRequestPermissionsFromFragment(Fragment fragment, String permission, int serviceCode) {
        int permissionToGrantValue = ContextCompat.checkSelfPermission(context, permission);
        if (permissionToGrantValue != PackageManager.PERMISSION_GRANTED) {
            String[] stringArray = new String[] { permission };
            fragment.requestPermissions(stringArray, serviceCode);
            return false;
        }
        return true;
    }
    ...
    public void onRequestPermissionResult(int requestCode, String permissions[], int[] grantResults, Runnable runnable) {

//        switch (requestCode) {
//            case REQUEST_ID_MULTIPLE_PERMISSIONS_SERVICE_CODE:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    HarmUtils.showToast("permission granted", context);
                    runnable.run();
                } else {
                    HarmUtils.showToast("Permission Denied, You cannot access storage data. Enable it from settings.!", context);
                    HarmUtils.openAppInfoSettings(context);
                }
//                break;
//        }
